...
import { RefineParams } from '../types/types';
  ...
  class JsonExtractRequestDto {
    @ApiProperty({description: 'text to extract structured data from'})
    @IsNotEmpty()
    text: string;
  ...
  class SchemaRequestDto {
    @ApiProperty({ description: 'json schema to use as model for data extraction' })
    @IsJSON()
    jsonSchema: string;
  ...
  class ExampleRequestDto {
    @ApiProperty({ description: 'example input text' })
    @IsNotEmpty()
    exampleInput: string;
  
    @ApiProperty({ description: 'example of desired json output' })
    @IsJSON()
