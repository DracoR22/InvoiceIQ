...
import { ChainCall, DebugReport, LlmCall } from "../dtos/debug.dto";
...
export class DebugCallbackHandler extends BaseCallbackHandler {
    ...
    async handleChainEnd(outputs: ChainValues, runId: string): Promise<void> {
        const endedChain = this._debugReport.chains.find((chain) => chain.runId === runId)

        endedChain.end.outputs = outputs
    }

    async handleChainError(err: any, runId: string): Promise<void> {
        const erroredChain = this._debugReport.chains.find((chain) => chain.runId === runId)
