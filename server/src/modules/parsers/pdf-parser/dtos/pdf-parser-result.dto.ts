...
class UploadResultDto {
    @ApiProperty({ description: 'Original file name of the uploaded file' })
    originalFileName: string
}

export class PdfParserResultDto {
    @ApiProperty({ description: 'Parsed and post-processed content of the PDF file' })
    content: string
}

class UrlResultDto {
    @ApiProperty({ description: 'Original URL of the PDF file' })
    @IsUrl()
    originalUrl: string
}
